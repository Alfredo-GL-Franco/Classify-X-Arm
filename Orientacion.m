...
td=[30 0 15]';
theta=pi/2;
 Rd=[0  0  1; cos(theta) sin(theta) 0 ; cos(theta)    -sin(theta)    0 ];
% Rd=[1 0 0 ; 0 cos(theta) -sin(theta) ; 0    sin(theta)    cos(theta) ];
K=[1,0,0;
    0,1,0;
    0,0,1];    
Kg=diag([5,5,5,5,5,5]);
lambda = 0.01; % Parámetro de regularización
...
T12=[[cos(theta_2), sin(theta_2), 0,    12.0*cos(theta_2)];
    [sin(theta_2),  -cos(theta_2),     0,    12.0*sin(theta_2)];
    [0,            0,               -1.0,  0];
    [0,            0,               0,    1.0]];

T23=[[cos(theta_3), 0,    sin(theta_3), 0];
...
clear arduinoObj
 ...
 % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % 
 % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % 
 % % % % % % Abrir y cerrar el griper%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Definir la posición objetivo donde se debe abrir la garra
 % Coordenadas deseadas

% % Verificar si se ha alcanzado la posición objetivo
% if norm(T05(;3) - td) < 0.5  % Compara con una tolerancia pequeña
%     % Enviar comando al Arduino para abrir la garra (ejemplo: 'G:1' para abrir)
%     writeline(arduinoObj, "G:1");  
%     disp("Garra abierta");
% end
